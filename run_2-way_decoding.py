...
Recognition task (two-way classification)
...
def classify(clf,X_test,y_true):
    ...
    try:
        ...
        roc_auc = roc_auc_score(y_true, y_prob[:,1]) # added [:,1] removed multiclass
    except:
        y_prob = None
        roc_auc = np.nan
    
    acc = accuracy_score(y_true, y_pred)
    cm = confusion_matrix(y_true, y_pred) # removed labels
...
def write_results(classifier, mask, vp, acc, roc_auc, cm):        
    ...
    import csv
    fname = 'results_2-way_decoding.csv' 
    header = ['classifier','mask','leftout-subject', 'accuracy','roc-auc','confusion_matrix','classification']
    data = [classifier, mask, vp, acc, roc_auc, cm, classification]
...
#### process data
    ...
    label_dict = {labels[0]  : 0,
                  ...
                  labels[2]  : 2,
                  labels[3]  : 3}
    label_original = np.array([x.split('_')[-1][8:-4] for x in allfname])
    label_original_num = np.array([label_dict[x] for x in label_original], dtype='int')


    # make new labels for recognition
    for classification in ['backing','drums','vocals']: 
        label_new = np.zeros(np.shape(label_original_num)) 
        label_new[(label_original == 'merged') | (label_original == classification)] = 1 
...
#### RF and SVM
        ...
        for vp in np.unique(label_subject):
            ...
            y_train = label_new[label_subject != vp]
            
            Xte = allX[label_subject == vp, :]
            y_true = label_new[label_subject == vp]   
    ...
    #### CNN
            ...
            y_train = label_new[(label_subject != vp) & (label_subject != valvp0) & (label_subject != valvp1)] # use numeric labels for onehot later
            
            Xva = allX[(label_subject == valvp0) | (label_subject == valvp1), :]
            y_validation = label_new[(label_subject == valvp0) | (label_subject == valvp1)]   
            ...
            y_train = keras.utils.to_categorical(y_train, len(np.unique(label_new)))
            y_validation = keras.utils.to_categorical(y_validation, len(np.unique(label_new)))
            y_true = keras.utils.to_categorical(y_true, len(np.unique(label_new)))
